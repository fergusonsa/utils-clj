...
;; lein repl :connect <desired-server>:4009
;;
;; OR
;;
...
(use '[cenx.heimdallr.customers.shared.vzw.debug :exclude [clear raise flip force-raise]])
...
(defn show-alarm-info ([] (show-alarm-info (get-all-active-alarms)))
  ([alrms] (println "Currently " (count alrms) " alarms!\n") (doseq [alarm-name (apply sorted-set alrms)]     
      (if-let [alarm-obj (get-object alarm-name)] (println (format "%-100s" alarm-name) " updated " (format-date (:fault/timestamp alarm-obj)) " text "  (:fault/details alarm-obj))))))

(defn is-my-alarm [alarm-name] (or (contains? @submitted-alarms alarm-name) (let [alarm-obj (get-object alarm-name)] (if-let [txt (:fault/details alarm-obj)] (= txt scotts-description) false))))


(defn change-alarm
  ([alrm-name] (change-alarm alrm-name :Failed scotts-description))
  ([alrm-name state] (change-alarm alrm-name state scotts-description))
...
(defn toggle-my-alarm [alarm-id] (if-let [alarm-obj (get-object alarm-id)] (if (= (:fault/state alarm-obj) :Failed) (change-alarm alarm-id :Ok) (change-alarm alarm-id :Failed))))

(defn cancel-all-alarms [] (doseq [alarm-name (get-all-active-alarms)] (change-alarm alarm-name :Ok)))

(defn clear-alarm [alarm-name] (if (is-my-alarm alarm-name) (change-alarm alarm-name :Ok)) (display-num-alarms))
    ...
    (while (< (count @submitted-alarms) (+ init-subs num-to-submit))
      (let [alrm-name (rand-nth (get-alarm-names))]
        (if (and (not (nil? alrm-name))  (not (some #(=  alrm-name %) (get-all-active-alarms))))
          (change-alarm alrm-name :Failed)))))
  (display-num-alarms))
  
(defn cancel-all-my-alarms []
  (doseq [alarm-name (clojure.set/intersection @submitted-alarms (into #{} (get-all-active-alarms)))] (change-alarm alarm-name :Ok))
  (display-num-alarms))

(defn cancel-all-my-alarms2 [] (doseq [alarm-name (filter is-my-alarm (get-all-active-alarms))] (change-alarm alarm-name :Ok)))
...
(defn raise-alarm-of-type [alarm-type] (let [alrm-name (rand-nth (filter-alarm-names alarm-type))] (change-alarm alrm-name :Failed)))    
    ...
    (while (and (> (count @submitted-alarms) 0) (> (count @submitted-alarms) (- init-subs num-to-clear)) (> (count (get-all-active-alarms)) 0))
      (let [alarm-name (rand-nth (seq (clojure.set/intersection @submitted-alarms (into #{} (get-all-active-alarms)))))]
        (if (not (nil? alarm-name))
          (change-alarm alarm-name :Ok)))))
  (display-num-alarms))

(defn find-my-alarms [] (reset! submitted-alarms (into #{} (filter is-my-alarm (get-all-active-alarms)))))

(defn get-display-name [id] (if (sequential? id) (map get-display-name id) (let [obj (get-object id)] (:eml3/displayName obj))))
